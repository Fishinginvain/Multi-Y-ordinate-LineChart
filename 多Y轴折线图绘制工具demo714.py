...
class MyFrame(wx.Frame):
    ...
    def draw_figure(self):
        ...
        ax.tick_params(axis='y', colors=colors[0], labelsize=12)
        lines = []
        labels = []
        ...
        for filename, checked in self.checkbox_states.items():
            if checked:
                ...
                if j == 0:
                    ...
                    ax_new.tick_params(axis='y', colors=colors[j % len(colors)], labelsize=12)
                    ...
                    # 压缩图表
                    ax_new.figure.subplots_adjust(right=0.95 - (j - 1) * 0.05)
                lines += line
                labels.append(filename)
                j += 1
        # 图例
        legend = self.figure.legend(lines, labels, loc='upper right', fontsize=12, frameon=False)
