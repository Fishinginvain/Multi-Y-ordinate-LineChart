...
class MyFrame(wx.Frame):
    def __init__(self):
        ...
        # 添加按钮到工具栏
        self.highlight_button = wx.Button(self.toolbar, label="开启高亮")
        self.highlight_button.Bind(wx.EVT_BUTTON, self.toggle_highlight)
        self.toolbar.AddControl(self.highlight_button)
        self.highlight_active = False  # 高亮功能状态
        self.tooltip = None  # 初始化工具提示
        ...
        for filename, checked in self.checkbox_states.items():
            if checked:
                ...
                if j == 0:
                    ...
                    ax.yaxis.label.set_color(self.file_colors[filename])
                    ax.tick_params(axis='y', colors=self.file_colors[filename], labelsize=12)
                    line = ax.plot(df.index, df['value'], label=filename, color=self.file_colors[filename], linewidth=1)  # 初始线宽为1
                    ...
                    line = ax_new.plot(df.index, df['value'], label=filename, color=self.file_colors[filename], linewidth=1)  # 初始线宽为1
    ...
    def toggle_highlight(self, event):
        ...
        if self.highlight_active:
            self.highlight_button.SetLabel("关闭高亮")
        else:
            self.highlight_button.SetLabel("开启高亮")
            if self.tooltip:
    ...
    def reset_lines(self):
        for ax in self.figure.get_axes():
            for line in ax.get_lines():
                line.set_linewidth(1)  # 初始线宽为1
                line.set_alpha(1.0)  # 保持基础颜色不变
        self.figure.canvas.draw()
        ...
        for ax in self.figure.get_axes():
            for line in ax.get_lines():
                if line.contains(event)[0]:
                    line.set_linewidth(1.5)  # 调整加粗程度为1.5
                    line.set_alpha(1.0)
                    # 显示折线信息
                    tooltip_text = f"{line.get_label()}\n开始时间: {line.get_xdata()[0]}\n结束时间: {line.get_xdata()[-1]}"
                    if not self.tooltip:
                        self.tooltip = wx.PopupWindow(self, wx.SIMPLE_BORDER)
                        tooltip_text_ctrl = wx.StaticText(self.tooltip, label=tooltip_text)
                        sizer = wx.BoxSizer(wx.VERTICAL)
                        sizer.Add(tooltip_text_ctrl, 0, wx.ALL, 5)
                        self.tooltip.SetSizer(sizer)
                        self.tooltip.Fit()
                    else:
                        self.tooltip.GetChildren()[0].SetLabel(tooltip_text)
                        self.tooltip.Fit()
                    self.tooltip.Position((event.guiEvent.GetX() + 10, event.guiEvent.GetY() + 10), (0, 0))
                    self.tooltip.Show()
                    found = True
                else:
                    line.set_linewidth(1)  # 不满足高亮条件则线宽为1
                    line.set_alpha(1.0)  # 保持基础颜色不变
        if not found and self.tooltip:
            self.tooltip.Destroy()
            self.tooltip = None
        self.figure.canvas.draw()
